// Data layer for search — interfaces, data loading, category definitions,
// rendering functions. NO DOM access. Imported by page.ts (UI) and chat/page.ts.
...
import { formatTagsClickable } from "../dice/tags";
import {
  getLocalIndexFile,
...
export function getEnabledLibraryBases(): string[] {
  try {
...
export function dataBase(_lang: Language): string {
...
export function categoryInfo(c: number): CategoryInfo {
...
export function srcCode(s: Entry["s"]): string {
...
export let indexCache: IndexFile | null = null;
...
export function search(query: string, idx: IndexFile, opts: FilterOpts): Entry[] {
...
export const dataCache = new Map<string, DataEntry[]>();
export const dataPending = new Map<string, Promise<DataEntry[]>>();
